...
int DhcpClass::beginWithDHCP(uint8_t *mac, const char *hostname, unsigned long timeout, unsigned long responseTimeout)
{
  ...
  if (hostname == nullptr)
  {
    strlcpy(_dhcpHostname, DEFAULT_HOST_NAME, MAX_HOST_NAME_LENGTH - 5);
...
{
  uint8_t buffer[20 + MAX_HOST_NAME_LENGTH];
  memset(buffer, 0, 20 + MAX_HOST_NAME_LENGTH);
  IPAddress dest_addr(255, 255, 255, 255); // Broadcast address

  if (_dhcpUdpSocket.beginPacket(dest_addr, DHCP_SERVER_PORT) == -1)
  {
    //Serial3.printf("DHCP transmit error\n");
